...
class OBJudge:
    ...
    @property
    def pi(self):
        # Lazy: sympy.parsing.latex needs antlr4-python3-runtime>=4.11. In envs pinned
        # to 4.9.3 (LF/omegaconf 2.3.0), construct only when actually substituting \pi
        # so that __init__ never blows up.
        if self._pi is None:
