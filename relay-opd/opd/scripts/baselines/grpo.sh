#!/usr/bin/env bash
# Pure GRPO baseline on a prompt-only math dataset.
set -xeuo pipefail
...
echo "[GRPO] experiment=${EXP_ID} output=${OUTPUT_DIR}"
echo "[GRPO] student=${STUDENT_MODEL} data=${TRAIN_DATA}"
echo "[GRPO] batch=${train_batch_size} rollout_n=${rollout_n} response=${max_response_length}"
...
echo "=== GRPO training complete: ${EXP_ID} ==="
