...
class FSDPEngine(BaseEngine):
    ...
    def forward_backward_batch(self, data: TensorDict, loss_function: Callable, forward_only=False) -> list[TensorDict]:
        ...
        batch_num_tokens_override = tu.get_non_tensor_data(data, "batch_num_tokens_override", default=None)
...
class FSDPEngineWithLMHead(FSDPEngine):
    def prepare_model_inputs(self, micro_batch: TensorDict):
        ...
        label_input_ids = micro_batch.get("pg_action_input_ids", input_ids)
        ...
        if use_remove_padding:
            ...
            label_ids_rmpad = label_input_ids.values().unsqueeze(0)  # (1, total_nnz)
            input_ids_rmpad_rolled = torch.roll(label_ids_rmpad, shifts=-1, dims=1)  # (1, total_nnz)
            ...
            if pad_mode == DatasetPadMode.NO_PADDING:
                ...
                input_ids_rmpad_rolled = torch.roll(label_input_ids.values(), shifts=-1, dims=0)
                output_args["input_ids_rmpad_rolled"] = input_ids_rmpad_rolled
    ...
    def forward_step(self, micro_batch: TensorDict, loss_function, forward_only):
        ...
        with autocast_ctx:
            ...
            if forward_only or tu.get_non_tensor_data(micro_batch, "return_model_output", default=False):
