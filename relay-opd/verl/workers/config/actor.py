...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        gradient_accumulation_steps (int): Number of sample chunks to accumulate before one optimizer step.
    ...
    def validate(self, n_gpus: int, train_batch_size: int, model_config: dict = None):
        ...
        if not self.use_dynamic_bsz:
            if self.gradient_accumulation_steps < 1:
                raise ValueError(
                    f"[actor] gradient_accumulation_steps must be >= 1, got {self.gradient_accumulation_steps}"
                )

            if self.ppo_mini_batch_size % self.gradient_accumulation_steps != 0:
                raise ValueError(
                    f"[actor] ppo_mini_batch_size ({self.ppo_mini_batch_size}) must be divisible by "
