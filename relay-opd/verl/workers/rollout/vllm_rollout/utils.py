...
class vLLMColocateWorkerExtension:
    ...
    def _use_opd_draft_only_sync(self):
        return os.environ.get("VERL_OPD_SPECULATIVE_ENABLE") == "1" and self._get_drafter_model() is not None

    def _iter_weight_update_models(self):
        if self._use_opd_draft_only_sync():
            yield self._get_drafter_model()
        else:
            yield from self._iter_all_models()

    def _iter_weight_update_models_with_config(self):
        if self._use_opd_draft_only_sync():
            draft_cfg = self._get_draft_model_config()
            if draft_cfg is not None:
                yield self._get_drafter_model(), draft_cfg
        else:
            yield from self._iter_all_models_with_config()
    ...
    def update_weights_from_ipc(self, peft_config: dict = None, base_sync_done=False, use_shm: bool = False):
        ...
        if self._is_qat_model:
            ...
            for model in self._iter_weight_update_models():
        ...
        if self._is_qat_model:
            ...
            for model, model_config in self._iter_weight_update_models_with_config():
...
def extract_prompt_logprobs(
    ...
    When include_token_logprobs=True and num_prompt_logprobs > 0, also extracts
    the actual token's logprob into result_dict["prompt_token_logprobs"].
    This requires prompt_token_ids to identify the actual token at each position.
    """
    if num_prompt_logprobs is None:
        return

    prompt_logprobs_ls, prompt_ids_ls = [], []
    prompt_token_logprobs_ls = [] if include_token_logprobs else None
    # NOTE: logprob of first prompt token is None.
    for pos, logprobs_dict in enumerate(output.prompt_logprobs[1:]):
        if num_prompt_logprobs == 0:
            ...
            if include_token_logprobs:
                prompt_token_logprobs_ls.append(logprob)
        else:
            ...
            if include_token_logprobs and prompt_token_ids is not None:
                actual_id = prompt_token_ids[pos + 1]
                actual_entry = logprobs_dict.get(actual_id) or logprobs_dict.get(str(actual_id))
                if actual_entry is None:
                    raise RuntimeError(
                        f"actual token {actual_id} missing from prompt_logprobs at pos {pos}; "
                        f"num_prompt_logprobs={num_prompt_logprobs}, "
                        f"dict_keys={list(logprobs_dict.keys())[:8]}"
                    )
                prompt_token_logprobs_ls.append(actual_entry.logprob)
    ...
    if include_token_logprobs and prompt_token_logprobs_ls is not None:
        prompt_token_logprobs_ls.append(0.0)
        result_dict["prompt_token_logprobs"] = prompt_token_logprobs_ls
