...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def _assign_train_defaults(self, data: TensorDict):
        default_keys = dict(
    ...
    def _global_loss_token_count(self, data: TensorDict) -> float:
    ...
    def _merge_train_outputs(self, output_lst: list[dict]) -> dict:
        output = {"loss": [], "metrics": {}}
        ...
        for micro_output in output_lst:
            ...
            losses = micro_output.get("loss", [])
            if not isinstance(losses, list):
                losses = [losses]
            output["loss"].extend(losses)

            metrics = micro_output.get("metrics", {})
            append_to_dict(output["metrics"], metrics)

        return output

    def _train_batch_accumulated(self, data: TensorDict, gradient_accumulation_steps: int) -> TensorDict:
        ...
        assert data.shape[0] % gradient_accumulation_steps == 0, (
            f"local mini-batch size ({data.shape[0]}) must be divisible by "
        ...
        accum_batch_size_per_gpu = data.shape[0] // gradient_accumulation_steps

        self._assign_train_defaults(data)
        batch_num_tokens = self._global_loss_token_count(data)

        accum_dataloader = tu.make_iterator(
            data,
            mini_batch_size=accum_batch_size_per_gpu,
            epochs=1,
            seed=None,
        ...
        with Timer(name="train_batch", logger=None) as timer:
            for accum_td in accum_dataloader:
                self._assign_train_defaults(accum_td)
                tu.assign_non_tensor(
                    accum_td,
                    batch_num_tokens_override=batch_num_tokens,
                    disable_auto_offload=True,
                )
                output = self.engine.forward_backward_batch(accum_td, self.loss_fn, forward_only=False)
        ...
        if self.engine.is_mp_src_rank_with_outputs():
            output = self._merge_train_outputs(output_lst)
    ...
    def train_mini_batch(self, data: TensorDict) -> TensorDict:
        ...
        gradient_accumulation_steps = int(tu.pop(data, key="gradient_accumulation_steps", default=1))

        assert mini_batch_size is not None or num_mini_batch is not None
        assert gradient_accumulation_steps >= 1, f"Got {gradient_accumulation_steps=}"
        ...
        with (
            ...
            for batch_idx, mini_batch_td in enumerate(dataloader):
                global_token_num = self._global_token_num(mini_batch_td)
                ...
                if gradient_accumulation_steps > 1:
                    actor_output = self._train_batch_accumulated(mini_batch_td, gradient_accumulation_steps)
                else:
