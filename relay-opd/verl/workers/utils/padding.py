...
def left_right_2_no_padding(data: TensorDict) -> TensorDict:
    ...
    pg_action_input_ids = data.pop("pg_action_input_ids", None)
    if pg_action_input_ids is not None:
        pg_action_input_ids_rmpad = index_first_axis(pg_action_input_ids.unsqueeze(-1).flatten(0, 1), indices)
        data["pg_action_input_ids"] = torch.nested.nested_tensor_from_jagged(
            pg_action_input_ids_rmpad.squeeze(-1), offsets=cu_seqlens
        )
    ...
    # (bsz, seqlen, topk) or (bsz, seqlen, 1) for k1
    ...
    if teacher_logprobs is not None and teacher_ids is not None:
        ...
        if teacher_ids.dim() != 3 or teacher_logprobs.dim() != 3:
            raise RuntimeError(
                f"Expected teacher_ids/teacher_logprobs to be [B,S,K], "
                f"got teacher_ids={teacher_ids.shape} teacher_logprobs={teacher_logprobs.shape}"
            )
        if teacher_ids.shape != teacher_logprobs.shape:
            raise RuntimeError(
                f"teacher_ids and teacher_logprobs shape mismatch: "
                f"teacher_ids={teacher_ids.shape} teacher_logprobs={teacher_logprobs.shape}"
            )
        if teacher_ids.shape[:2] != attention_mask.shape:
            raise RuntimeError(
                f"teacher top-k tensors must align with attention_mask: "
                f"teacher_ids.shape[:2]={teacher_ids.shape[:2]}, "
    ...
    # (bsz, seqlen) — actual-token teacher logprob for hybrid k1 PG
    teacher_token_logprobs = data.get("teacher_token_logprobs", None)
    if teacher_token_logprobs is not None:
        teacher_token_logprobs_rmpad = index_first_axis(
            teacher_token_logprobs.unsqueeze(-1).flatten(0, 1), indices
        )
        data["teacher_token_logprobs"] = torch.nested.nested_tensor_from_jagged(
            teacher_token_logprobs_rmpad.squeeze(-1), offsets=cu_seqlens
        )
