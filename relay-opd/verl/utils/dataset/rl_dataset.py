...
class OfflineTeacherTrajectoryDataset(Dataset):
    """Replay offline teacher trajectories as supervised/distillation responses."""
    ...
    def __init__(
        ...
        self.max_samples = max_samples
        self.cache_dir = os.path.expanduser(config.get("cache_dir", "~/.cache/verl/rlhf"))
        offline_cfg = config.get("offline_teacher_trajectory", {})
        self.prompt_token_key = offline_cfg.get("prompt_token_key", "prompt_token_ids")
        self.teacher_prompt_token_key = offline_cfg.get("teacher_prompt_token_key", None)
        self.response_token_key = offline_cfg.get("response_token_key", "response_token_ids")
        self.gt_key = offline_cfg.get("gt_key", "gt")
        self.data_source = offline_cfg.get("data_source", "offline_teacher")
        self.max_prompt_length = config.get("max_prompt_length", 1024)
        self.max_response_length = config.get("max_response_length", 1024)
        self.filter_overlong = offline_cfg.get("filter_overlong", True)
        self.sample_idx_key = offline_cfg.get("sample_idx_key", "sample_idx")
        self.sample_idx_value = offline_cfg.get("sample_idx_value", None)
        self.unique_key = offline_cfg.get("unique_key", None)
        self.shuffle = config.get("shuffle", False)
        self.seed = config.get("seed")
        self._download()
        self._read_files()
    ...
    def _read_files(self):
        ...
        for data_file in self.data_files:
            ...
            dataframes.append(dataframe)
        self.dataframe: datasets.Dataset = datasets.concatenate_datasets(dataframes)
        print(f"offline teacher trajectory dataset len: {len(self.dataframe)}")

        if self.sample_idx_value is not None:
            sample_idx_value = int(self.sample_idx_value)
            before = len(self.dataframe)
            self.dataframe = self.dataframe.filter(
                lambda row: int(row.get(self.sample_idx_key, -1)) == sample_idx_value,
                desc=f"Filtering offline trajectories with {self.sample_idx_key}={sample_idx_value}",
            )
            print(
                f"offline teacher trajectory {self.sample_idx_key}={sample_idx_value} len: "
                f"{len(self.dataframe)} (from {before})"
            )

        if self.unique_key:
            before = len(self.dataframe)
            self.dataframe = self._select_first_per_unique_key(self.dataframe, self.unique_key)
            print(f"offline teacher trajectory unique {self.unique_key} len: {len(self.dataframe)} (from {before})")

        if self.filter_overlong:
            self.dataframe = self.dataframe.filter(
                lambda row: len(row[self.prompt_token_key]) <= self.max_prompt_length
                and len(row[self.response_token_key]) <= self.max_response_length,
                desc=(
                    f"Filtering offline trajectories longer than prompt={self.max_prompt_length} "
                    f"or response={self.max_response_length}"
                ),
            )
            print(f"offline teacher trajectory filtered len: {len(self.dataframe)}")
        ...
        if self.max_samples > 0 and self.max_samples < total:
            if self.shuffle:
                ...
                indices = np.arange(self.max_samples)
            self.dataframe = self.dataframe.select(indices.tolist())
            print(f"selected {self.max_samples} offline trajectories out of {total}")
    ...
    def _to_hashable(value):
        ...
        if isinstance(value, list | tuple):
            return tuple(OfflineTeacherTrajectoryDataset._to_hashable(item) for item in value)
        if isinstance(value, dict):
            return tuple(
                sorted((key, OfflineTeacherTrajectoryDataset._to_hashable(item)) for key, item in value.items())
            )
        return value

    @classmethod
    def _select_first_per_unique_key(cls, dataframe: datasets.Dataset, unique_key: str) -> datasets.Dataset:
        if unique_key not in dataframe.column_names:
            raise KeyError(f"offline teacher trajectory unique_key={unique_key!r} not found in columns")

        seen = set()
        indices = []
        for idx, row in enumerate(dataframe):
            key = cls._to_hashable(row[unique_key])
            if key in seen:
                continue
            seen.add(key)
            indices.append(idx)
        return dataframe.select(indices)
    ...
    def __getitem__(self, item):
        ...
        prompt_ids = [int(x) for x in normalize_token_ids(row[self.prompt_token_key])]
        teacher_prompt_ids = None
        if self.teacher_prompt_token_key and self.teacher_prompt_token_key in row:
            teacher_prompt_ids = [int(x) for x in normalize_token_ids(row[self.teacher_prompt_token_key])]
        response_ids = [int(x) for x in normalize_token_ids(row[self.response_token_key])]
        gt = row.get(self.gt_key, row.get("answer", None))
        unique_id = row.get("unique_id", item)
        result = {
            "offline_prompt_token_ids": prompt_ids,
            "offline_response_token_ids": response_ids,
            "offline_teacher_response_token_ids": response_ids,
            "raw_prompt": row.get("messages", []),
            "data_source": row.get("data_source", self.data_source),
            "reward_model": {"ground_truth": gt},
            "extra_info": {"index": item, "unique_id": unique_id, "sample_idx": row.get("sample_idx", None)},
            "index": item,
            "dummy_tensor": torch.tensor([0], dtype=torch.uint8),
        }
        if teacher_prompt_ids is not None:
            result["offline_teacher_prompt_token_ids"] = teacher_prompt_ids
        return result
    ...
    def split(self, num_splits: int):
        ...
        for i in range(num_splits):
            start_idx = i * split_size
            end_idx = (i + 1) * split_size if i < num_splits - 1 else total_samples
            split_dataset = OfflineTeacherTrajectoryDataset(
                data_files=self.data_files,
                tokenizer=self.tokenizer,
                config=self.config,
                processor=self.processor,
            ...
            split_dataset.dataframe = self.dataframe.select(range(start_idx, end_idx))
            split_dataset.original_data_files = self.original_data_files
            splits.append(split_dataset)
        return splits
