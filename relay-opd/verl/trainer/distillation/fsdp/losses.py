# Copyright 2025 Bytedance Ltd. and/or its affiliates
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.


import torch
import torch.nn.functional as F

from verl.utils.ulysses import (
    get_ulysses_sequence_parallel_world_size,
    slice_input_tensor,
)
from verl.workers.config import DistillationConfig, DistillationLossConfig


def kl_divergence(log_q: torch.Tensor, log_p: torch.Tensor) -> torch.Tensor:
    """Compute KL divergence between two distributions given their log probabilities."""
    log_p = log_p.float()
    log_q = log_q.float()
    p = log_p.exp()
    support = p > 0
    safe_log_p = torch.where(support, log_p, torch.zeros_like(log_p))
    safe_log_q = torch.where(support, log_q, torch.zeros_like(log_q))
    kld = torch.where(support, p * (safe_log_p - safe_log_q), torch.zeros_like(p))
    return kld.sum(dim=-1)


def _prepare_topk_tensors(
    student_logits: torch.Tensor,
    teacher_topk_log_probs: torch.Tensor,
    teacher_topk_ids: torch.Tensor,
    config: DistillationConfig,
) -> tuple[torch.Tensor, torch.Tensor]:
    assert teacher_topk_log_probs.is_nested and teacher_topk_ids.is_nested
    teacher_topk_log_probs = teacher_topk_log_probs.values().unsqueeze(0)  # (1, total_nnz, topk)
    teacher_topk_ids = teacher_topk_ids.values().unsqueeze(0).long()  # (1, total_nnz, topk)

    if student_logits.dim() != 3:
        raise RuntimeError(f"student_logits must be 3D [1,total_nnz,vocab], got {student_logits.shape}")
    if teacher_topk_ids.dim() != 3:
        raise RuntimeError(f"teacher_topk_ids must be 3D [1,total_nnz,K], got {teacher_topk_ids.shape}")
    if teacher_topk_log_probs.shape != teacher_topk_ids.shape:
        raise RuntimeError(
            f"teacher shape mismatch: ids={teacher_topk_ids.shape} logprobs={teacher_topk_log_probs.shape}"
        )
    expected_topk = getattr(config.distillation_loss, "topk", None)
    if expected_topk is not None and teacher_topk_ids.shape[-1] != expected_topk:
        raise RuntimeError(
            f"teacher_topk_ids last dim must equal topk={expected_topk}, "
            f"got {teacher_topk_ids.shape[-1]}"
        )

    # Split across SP groups: (bsz, seqlen, topk) -> (bsz, seqlen/sp_size, topk).
    if get_ulysses_sequence_parallel_world_size() > 1:
        teacher_topk_log_probs = slice_input_tensor(teacher_topk_log_probs, dim=1)
        teacher_topk_ids = slice_input_tensor(teacher_topk_ids, dim=1)
    if teacher_topk_ids.shape[:2] != student_logits.shape[:2]:
        raise RuntimeError(
            f"shape mismatch before gather: teacher_topk_ids={teacher_topk_ids.shape}, "
            f"student_logits={student_logits.shape}"
        )
    return teacher_topk_log_probs, teacher_topk_ids


def _compute_overlap_outputs(
    teacher_topk_ids: torch.Tensor,
    student_topk_ids: torch.Tensor,
    token_kl: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    # Diagnostics for tracking teacher/student top-k overlap in OPD, following
    # "Rethinking On-Policy Distillation of Large Language Models" (arXiv:2604.13016).
    overlap_mask = (teacher_topk_ids.unsqueeze(-1) == student_topk_ids.unsqueeze(-2)).any(dim=-1)
    overlap_count = overlap_mask.sum(dim=-1)
    overlap_token_advantage_sum = (-token_kl * overlap_mask).sum(dim=-1)
    overlap_token_advantage = overlap_token_advantage_sum / overlap_count.clamp_min(1)
    overlap_token_advantage = torch.where(
        overlap_count > 0, overlap_token_advantage, torch.zeros_like(overlap_token_advantage)
    )
    return overlap_count, overlap_token_advantage


def compute_forward_kl_topk(
    student_logits: torch.Tensor,
    teacher_topk_log_probs: torch.Tensor,
    teacher_topk_ids: torch.Tensor,
    config: DistillationConfig,
    data_format: str,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Compute forward KL distillation loss using top-k log probabilities.

    Args:
        student_logits: (bsz, seqlen/sp_size, vocab_size).
        teacher_topk_log_probs: (bsz, seqlen, topk).
        teacher_topk_ids: (bsz, seqlen, topk).
        data_format: "thd" or "bshd", models not support THD format, e.g GPT-OSS, Qwen3.5

    Returns:
    - distillation_losses: (bsz, seqlen/sp_size)
    - student_mass: (bsz, seqlen/sp_size)
    - teacher_mass: (bsz, seqlen/sp_size)
    """
    teacher_topk_log_probs, teacher_topk_ids = _prepare_topk_tensors(
        student_logits=student_logits,
        teacher_topk_log_probs=teacher_topk_log_probs,
        teacher_topk_ids=teacher_topk_ids,
        config=config,
    )
    # 2. compute token-wise KL divergence across sp groups
    student_log_probs = F.log_softmax(student_logits, dim=-1)
    student_topk_ids = torch.topk(student_log_probs, k=teacher_topk_ids.shape[-1], dim=-1).indices
    student_topk_log_probs = torch.gather(student_log_probs, dim=-1, index=teacher_topk_ids)
    student_mass = student_topk_log_probs.exp().sum(dim=-1)
    teacher_mass = teacher_topk_log_probs.exp().sum(dim=-1)
    loss_config: DistillationLossConfig = config.distillation_loss
    if loss_config.log_prob_min_clamp is not None:
        student_topk_log_probs = student_topk_log_probs.clamp_min(loss_config.log_prob_min_clamp)
        teacher_topk_log_probs = teacher_topk_log_probs.clamp_min(loss_config.log_prob_min_clamp)
    distillation_losses = kl_divergence(log_q=student_topk_log_probs, log_p=teacher_topk_log_probs)

    token_kl = teacher_topk_log_probs.exp() * (teacher_topk_log_probs - student_topk_log_probs)
    overlap_count, overlap_token_advantage = _compute_overlap_outputs(
        teacher_topk_ids=teacher_topk_ids,
        student_topk_ids=student_topk_ids,
        token_kl=token_kl,
    )

    return {
        "distillation_losses": distillation_losses,
        "student_mass": student_mass,
        "teacher_mass": teacher_mass,
        "overlap_count": overlap_count,
        "overlap_token_advantage": overlap_token_advantage,
    }

def compute_forward_kl_topk_teacher_renorm(
    student_logits: torch.Tensor,
    teacher_topk_log_probs: torch.Tensor,
    teacher_topk_ids: torch.Tensor,
    config: DistillationConfig,
    data_format: str,
) -> dict[str, torch.Tensor]:
    """Compute teacher-renormalized top-k FKL.

    The teacher distribution is renormalized on its own top-k support, while
    the student probabilities are gathered from the full-vocabulary softmax.
    This keeps probability mass outside teacher top-k visible to the loss.
    """
    del data_format
    teacher_topk_log_probs, teacher_topk_ids = _prepare_topk_tensors(
        student_logits=student_logits,
        teacher_topk_log_probs=teacher_topk_log_probs,
        teacher_topk_ids=teacher_topk_ids,
        config=config,
    )
    teacher_topk_log_probs = F.log_softmax(teacher_topk_log_probs.float(), dim=-1)

    student_log_probs = F.log_softmax(student_logits, dim=-1)
    student_topk_ids = torch.topk(student_log_probs, k=teacher_topk_ids.shape[-1], dim=-1).indices
    student_topk_log_probs = torch.gather(student_log_probs, dim=-1, index=teacher_topk_ids)
    student_mass = student_topk_log_probs.exp().sum(dim=-1)
    teacher_mass = torch.ones_like(student_mass)

    loss_config: DistillationLossConfig = config.distillation_loss
    if loss_config.log_prob_min_clamp is not None:
        student_topk_log_probs = student_topk_log_probs.clamp_min(loss_config.log_prob_min_clamp)
        teacher_topk_log_probs = teacher_topk_log_probs.clamp_min(loss_config.log_prob_min_clamp)

    distillation_losses = kl_divergence(log_q=student_topk_log_probs, log_p=teacher_topk_log_probs)
    token_kl = teacher_topk_log_probs.exp() * (teacher_topk_log_probs - student_topk_log_probs)
    overlap_count, overlap_token_advantage = _compute_overlap_outputs(
        teacher_topk_ids=teacher_topk_ids,
        student_topk_ids=student_topk_ids,
        token_kl=token_kl,
    )

    return {
        "distillation_losses": distillation_losses,
        "student_mass": student_mass,
        "teacher_mass": teacher_mass,
        "overlap_count": overlap_count,
        "overlap_token_advantage": overlap_token_advantage,
    }
