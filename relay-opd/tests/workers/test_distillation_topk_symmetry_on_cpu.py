...
from verl.trainer.distillation.fsdp.losses import (
    ...
    compute_forward_kl_topk_teacher_renorm,
)
...
def test_teacher_renorm_topk_fkl_matches_manual_formula():
    logits = torch.tensor(
        [
            [0.0, 3.0, 1.0, -2.0, 0.5],
            [2.0, -1.0, 0.0, 4.0, 1.0],
    ...
    teacher_ids = _nested_from_rows([[1, 2, 4], [3, 0, 4]]).to(torch.int64)
    # These are full-vocab log probabilities restricted to teacher top-k.
    teacher_logprobs = _nested_from_rows(
        [
            [torch.log(torch.tensor(0.55)), torch.log(torch.tensor(0.25)), torch.log(torch.tensor(0.05))],
            [torch.log(torch.tensor(0.50)), torch.log(torch.tensor(0.10)), torch.log(torch.tensor(0.08))],
        ]
    ).to(torch.float32)
    config = SimpleNamespace(distillation_loss=SimpleNamespace(log_prob_min_clamp=None, topk=3))

    student_logprobs = torch.log_softmax(logits, dim=-1)
    student_topk_logprobs = torch.gather(student_logprobs, dim=-1, index=teacher_ids.values().unsqueeze(0))
    teacher_topk_logprobs = torch.log_softmax(teacher_logprobs.values().unsqueeze(0), dim=-1)

    expected_forward = (
        teacher_topk_logprobs.exp() * (teacher_topk_logprobs - student_topk_logprobs)
    ).sum(dim=-1)

    forward_output = compute_forward_kl_topk_teacher_renorm(
        student_logits=logits,
        teacher_topk_log_probs=teacher_logprobs,
        teacher_topk_ids=teacher_ids,
        config=config,
    ...
    torch.testing.assert_close(forward_output["distillation_losses"], expected_forward)
    torch.testing.assert_close(forward_output["student_mass"], student_topk_logprobs.exp().sum(dim=-1))
    torch.testing.assert_close(forward_output["teacher_mass"], torch.ones_like(expected_forward))
    assert forward_output["overlap_count"].shape == expected_forward.shape
    assert forward_output["overlap_token_advantage"].shape == expected_forward.shape
